    ...
    <v-row justify="center">
      <v-col cols="12" sm="12">
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          <v-row>
            <v-col cols="3">
              <input type="file" ref="file" name="image" @change="GetImage" />
              <img :src="avatar" width="100%" />
              <progress max="100" :value.prop="uploadPercentage"></progress>
              {{uploadPercentage}}
              <!-- <v-btn width="100%">Rotate Left</v-btn><v-btn width="100%">Rotate Right</v-btn> -->
            </v-col>
            <v-col cols="9">
              <v-row>
                <v-col cols="6">
    ...
    post: function() {
      ...
      if (this.$v.$invalid) {
        ...
        if (this.editedIndex > -1) {
          ...
          this.$http
            .put(`${server.path}contacts/update`, formData, {
              observe: "body"
            })
            .then(function(data) {
              console.log(data);
